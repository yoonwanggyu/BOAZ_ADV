...
async def llm_evaluation_node(state: ChatbotState) -> ChatbotState:
    ...
    print(f"최적화상태: {'완료' if optimization_completed else '진행중'}")
    print(f"재시도여부: {'예' if should_retry and not optimization_completed else '아니오'}")
...
async def merge_and_respond_node(state: ChatbotState) -> ChatbotState:
    ...
    try:
        ...
        if state.get('decision_slack', 'no').lower() == 'yes':
            ...
            text_to_send = f"<@{user_id_to_mention}> 님께 전달된 정보입니다.\n\n{final_answer}\n\n 정답 근거: \n\n{source_response.content}"
