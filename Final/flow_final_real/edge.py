...
def create_chatbot_graph():
    ...
    workflow.add_conditional_edges(
        "router_agent",
        route_after_router, # route_after_router는 flow_type을 그대로 반환
        {
            "neo4j_only": "neo4j_db",
    ...
    workflow.add_conditional_edges(
        ...
        route_after_neo4j,
        {
            "go_to_merge": "merge_and_respond",            # 'go_to_merge' 키워드 -> merge_and_respond 노드로
            "generate_vector_query": "generate_vector_query",
            "start_vector_db_flow": "adaptive_query_rewriter"   # 'start_vector_db_flow' 키워드 -> adaptive_query_rewriter 노드로
        }
    )
    ...
    # 6. VectorDB 검색/평가 루프의 조건부 분기 (항상 검색 후 평가에서 완료 판단)
    ...
    workflow.add_conditional_edges(
        ...
        route_after_evaluation,
        {
            ...
            "generate_answer": "merge_and_respond", # 'generate_answer' 키워드 -> merge_and_respond 노드로
        }
    )
